...
if __name__ == "__main__":
    ...
    argParser.add_argument('--jsonKey', help='Optional: compute similarity only on passed json metadata key')
                ...
                for point in global_minima[1][key]:
                    ...
                    if args.jsonKey:
                        ...
                        with open(point.filename) as json_file:
                            jsonDict = json.load(json_file)
                            if json_attr in jsonDict:
                                node["metadata"] =  json.dumps({json_attr : jsonDict[json_attr] })
                    else:
