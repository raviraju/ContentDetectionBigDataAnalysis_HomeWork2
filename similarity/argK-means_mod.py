...
if __name__ == "__main__":
    ...
    argParser.add_argument('--J', help='Optional: compute similarity only on passed json metadata')
        ...
        if args.J:
			...
			for eachFile in filterFiles(args.inputDir, args.accept):
				...
				with open(eachFile) as json_file:
					...
					if json_attr in jsonDict:
						#print json.dumps(jsonDict[json_attr], indent=4)
						list_of_points.append(Vector(eachFile, {json_attr : jsonDict[json_attr] } ))
        else:
			for eachFile in filterFiles(args.inputDir, args.accept):
        ...
        with open(args.outJSON, "w") as jsonF:
            ...
            for key in global_minima[1]:    #clusters
                ...
                for point in global_minima[1][key]:
                    ...
                    if args.J:
                        json_attr = args.J
                        with open(point.filename) as json_file:
                            jsonDict = json.load(json_file)
                            if json_attr in jsonDict:
                                node["metadata"] =  json.dumps({json_attr : jsonDict[json_attr] })#json.dumps(parser.from_file(point.filename)["metadata"])
                    else:
