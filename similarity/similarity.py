...
def main(argv = None):
	...
	try:
		try:
			opts, args = getopt.getopt(argv[1:], 'hvf:c:a:j:', ['help', 'verbose', 'directory=', 'file=', 'accept=', 'json=' ])
		...
		for filename in filename_list:
			...
			if json_input:
				...
				with open(filename) as json_file:
					file_parsed_data[filename_stripped] = json.load(json_file)
					union_feature_names = union_feature_names | set(file_parsed_data[filename_stripped].keys())
			else:
				try:
